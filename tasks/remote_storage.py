...
def get_job_metadata(job_uuid):
    ...
    try:
        ...
        logging.error('Error while gathering metadata:', exc)
...
def get_jobs():
    ...
    try:
        ...
        logging.error('Error while getting job directory list:', exc)
...
def create_root_jobs_index():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=100) as executor:
            todo = []
            for job in get_jobs():
                future = executor.submit(get_job_metadata, job)
                todo.append(future)
        ...
        logging.error('Error while creating root index:', exc)
...
def create_jobdir_metadata(uuid, pr_number, pr_author, task_name, returncode):
    """
...
class CloudUpload(FallibleTask):
    ...
    def _run(self):
        ...
        # we run 2 awscli commands so we can upload all "gzip" files with
        # correct encoding.
        self.execute_subtask(PopenTask(aws_sync_cmd + sync_all_except_gz))
        self.execute_subtask(PopenTask(aws_sync_cmd + sync_gz))

        # the following code is not critical for PRCI functionality.
        try:
            create_jobdir_metadata(self.uuid, self.pr_number, self.pr_author,
                                   self.task_name, self.returncode)
        except RuntimeError as exc:
            raise RuntimeError('Error while gathering metadata:', exc)

        # we list only "freeipa" repo PRs in root index
        # (Jobs of PRs against forks are not listed)
        if self.repo_owner == 'freeipa':
            try:
                self.execute_subtask(create_root_jobs_index())
            except RuntimeError as exc:
                raise RuntimeError('Error while creating root index:', exc)
